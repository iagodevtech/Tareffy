...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onToggle, isCollapsed = false, onCollapse }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={`${isOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 fixed md:static inset-y-0 left-0 z-50 w-72 ${isCollapsed ? 'md:w-16' : 'md:w-64'} bg-white border-r border-gray-200 transform transition-all duration-300 ease-in-out md:flex md:flex-shrink-0 shadow-lg`}>
              ...
              <nav className="mt-5 flex-1 px-2 space-y-1">
                {navigation.map((item) => (
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `group flex items-center ${isCollapsed ? 'px-3 py-2 justify-center' : 'px-2 py-2'} text-sm font-medium rounded-md transition-all duration-300 ${
                        isActive
